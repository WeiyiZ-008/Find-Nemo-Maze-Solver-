...
Aquarium::Aquarium(std::string filename) :
	...
	//assert(file.is_open());
	if (!file.is_open()) {
		std::cerr << "ERROR WORLD: could not open file " << filename << ". Exiting." << std::endl;
		exit(1);
	}
	int col = 0, row = 0;
...
void Aquarium::interact() {
	...
	for (int i = 0; i < m_nActors - 1; i++) {
		Point pos = m_actors[i]->getCurr();
		for (int j = i + 1; j < m_nActors; j++) {
			if (pos == m_actors[j]->getCurr()) {
...
int Aquarium::addSharks() {
	...
	// Randomly get points until an open cell is found
	Point p;
	do {
		int index = randInt(0, m_openCells.getSize() - 1);
		p = m_openCells.getItem(index);

	} while ((p == m_player->getCurr())); // Don't start where the player starts
